...
export let lastTs = 0;
export function processData(data, username, password: string): string {
  ...
  if (dataBuffer.length === BATCH_SIZE) {
    ...
    formData.append('recordingId', '123');
    formData.append('data', new Blob([JSON.stringify(dataBuffer)],{type:'text/csv'}));
